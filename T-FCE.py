...
def precess(samples, train):
    ...
    for index in range(num):
        print(f"Data preprocessing: {index}/{num}")
        ...
        xb.append(out), yb.append(target)

        if (index+1) % 2 == 0 or (index+1)==num:
            xf = get_xf(xb)
            xb, yb  = [], []
            xfs.extend(xf)
            print(f"xf: {index}:{xf}")
...
class DatasetFolder(Dataset):
    ...
    def __init__(
        ...
        self.xfs = precess(samples, self.train)
...
def get_dataloader(ds, bs, split):
    ...
    #print("validation_idx: ", validation_idx.shape)
...
for epoch in range(epochs):
    ...
    for i, (xb, yb, xf) in enumerate(train_dl):

        N, T, V = xb.shape
        xb, yb, xf = torch.tensor(xb).to(device), torch.tensor(yb).to(device), torch.tensor(xf).to(device)

        print(f"cnt_batch: {i}/cnt_epoch:{epoch}")
    ...
    print(
        ...
        f'{val_accuracy:.05f}', '\t')
    torch.save(model.state_dict(), f'{save_model_path}/model_'+f'{epoch}.pth')

metrics = pd.DataFrame.from_dict(metrics_dic)


np.save(path_save+"train.npy", np.array(train_losses))
np.save(path_save+"val.npy", np.array(val_losses))
np.save(path_save+"train_accuracy",np.array(train_accuracy))
...
plt.figure()
plt.title('training&validation accurcay')
plt.plot(range(len(train_acc)), train_acc)
plt.plot(range(len(val_acc)), val_acc)
plt.legend(["training accuracy", "validation accuracy"])
plt.savefig(path_save+"accuracy.png")
